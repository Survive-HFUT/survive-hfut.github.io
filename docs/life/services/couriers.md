...
- 宣区有两个代收点分别位于南区和北区（具体位置见下），且二者相距较远，因此别忘记写明南区还是北区
- 如果出现了写错南北区或是送错代收点的情况，可以和代收点处的工作人员联系，提供取件码或运单号，一般当天下午或第二天即可送回正确的代收点
...
- 寄明信片或是信函可以参考此邮编
- 对于寄相对贵重的信件或是想随时知道信件状态的同学可以在寄出前额外花 ￥3 办理挂号业务
...
- 信件通常由大活的言只书屋代收
- 言只书屋门口处有邮筒，每周一三五取件送信
  - 根据[邮政普遍服务营业场所名录查询](https://zwfw.spb.gov.cn/gjj/pfyycsml)，这可能不是由官方认可的邮政网点/邮筒，需要寄贵重信函、办理挂号或其他业务的同学可以去市中心的官方网点（如宣城市西林路营业所）咨询
...
1. 当快递送达并入库后，你可以在菜鸟裹裹/拼多多（需要绑定手机号）/商品物流页/短信通知查到取件码
   - 取件码格式类似于`xxx-y-zzzz`，其中`xxx`为货架号，`y`为在该货架的层数
2. 去代收点找到你快递所在的**货架**
3. 按**层数**找你的快递
4. 核对**取件码后四位、收货地址、电话尾号、姓名**是否与本人一致
5. 扫码出库，标记此快递已被取走
   - 部分代收点需要出示身份码，可在 菜鸟裹裹→我的→身份码 或 拼多多→聊天→物流消息→取快递→身份码 处找到

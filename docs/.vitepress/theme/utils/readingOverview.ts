...
import { canonicalArticlePath } from './articlePath.ts';
import type {
  ArticleReadingState,
  ArticleVersionsData,
  ReadingState,
...
export function buildReadingOverviewArticles(
  ...
  return Object.entries(versions)
    .map(([path, info]) => ({
      path: canonicalArticlePath(path),
      title: info.title,
      version: info.version,
      status: getReadingStatus(
        state.articles[canonicalArticlePath(path)],
        info.version,
      ),
      reading: state.articles[canonicalArticlePath(path)] ?? {},
    }))

...
**不要完全依赖 AI 来生成内容**，尤其是对于一些需要严谨表达的内容（如校规校纪、考试相关规定等），请务必仔细核对并确保准确无误，以免引起不必要的误解或麻烦

除此之外，AI 生成的内容语气十分奇怪，使用 AI 生成后务必进行人工润色修改
...
如果使用支持 Skills 的 AI 工具编写或润色文档，可以优先参考仓库内的 `hfut-doc-style` 技能：

```txt
$hfut-doc-style docs/待修改页面.md
```

该技能位于仓库的 `.agents/skills/hfut-doc-style` 目录下，主要用于减少 AI 生成味，并统一加粗、`<mark>`、Emoji、引用来源和提示块等写法
